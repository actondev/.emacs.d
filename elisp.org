...
** buttons for named block
   #+BEGIN_SRC emacs-lisp :results silent
(defmacro aod-button/make-org-babel-button (name &optional format)
  (button-lock-mode 1)
...
(defun aod-button/make-org-babel-buttons (&optional blocks)
  "Creates a button for all the named src blocks

   Pro tip: make an emacs-lisp src block calling this function
   and you will get a table with clickable buttons for all the
   src blocks"
  (let ((blocks (or blocks (org-babel-src-block-names))))
    (mapcar
     (lambda (name)
       ...
       (eval `(aod-button/make-org-babel-button ,name)))
     blocks)))
   #+END_SRC
